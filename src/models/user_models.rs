...
impl User {
    pub fn sign_up(username: String, display_name: String, password: String, role: String) -> Result<User, io::Error> {
        if User::is_username_exists(&username) {
            panic!("Username exists")
        }
        let user = User::create_object_from(username, display_name, password, role);
    ...
    fn get_user(username: &String) -> Result<User, io::Error> {
        ...
        let user_row_wrapper = users::table.filter(users::username.eq(&username)).first::<UserRowWrapper>(&connection).expect("User not found");
        Ok(User {
    ...
    fn create_object_from(username: String, display_name: String, password: String, role: String) -> User {
