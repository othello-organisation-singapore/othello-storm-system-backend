...
fn get_sorted_player_standings<'a>(player_ids: &HashSet<i32>, matches: &Vec<Box<dyn IGameMatch>>) -> Vec<PlayerStanding> {
...
fn get_major_scores_by_player_id(player_ids: &HashSet<i32>, matches: &Vec<Box<dyn IGameMatch>>) -> HashMap<i32, f64> {
    HashMap::from_iter(
        player_ids
            .iter()
